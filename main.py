...
def main():
    ...
    upload_photo_url = get_url_for_upload_photo(vk_token)
    upload_details = upload_photo_VK(Path(f'comics/{comics_image_name}'), upload_photo_url)
    saving_photo_details = save_photo_VK(vk_token, upload_details)
    publication_comics(vk_token, saving_photo_details['response'][0]['id'],
                       saving_photo_details['response'][0]['owner_id'], comics_comment)
